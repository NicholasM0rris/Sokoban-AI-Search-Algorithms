...
class SokobanMap:
    ...
    def apply_move(self, move):
        ...
        self.player_x = new_x
        self.player_y = new_y
        self.player_position = (self.player_x, self.player_y)
    ...
    def bfs(self):  # This is actually a BFS
        start = time.time()
        ...
        # print("adds new node2")
        for anaction in actions:  # add exploration results to the frontier.
            # print("adds new node3")
            # print(anaction)
            self.apply_move(anaction)
            self.state = (self.player_position, tuple(self.box_positions), tuple(self.tgt_positions))
            print(self.state)

            if tuple(self.state) not in visited_nodes:
                # print("adds new node4")
                if tuple(self.state) not in ft:
                    print("adds new node")
                    new_state = (self.player_position, tuple(self.box_positions), tuple(self.tgt_positions))
                    new_node = SokobanMap(depth=current_node.depth + 1, parent=current_node,
                                          action=anaction, player_position=self.player_position, box_positions=self.box_positions, tgt_positions=self.tgt_positions)
