            ...
            for y in range(height):
                ...
                # If a cell is touching three walls, or two cells with different x and y coords (not just above and below or left and right) then it's a corner
                    if len(adjacent_walls) >= 3 or (
                            len(adjacent_walls) == 2 and adjacent_walls[0][0] != adjacent_walls[1][0] and adjacent_walls[0][  #MAJOR CHANGE - [0][1] [1][1] was wrong way before and INDENT whole if condition
                        1] != adjacent_walls[1][1] and self.obstacle_map[cell[0]][cell[1]] is not self.OBSTACLE_SYMBOL):
                ...
                for b in range(a, len(corners)):
                    ...
                    if cell1 != cell2 and cell1[i] == cell2[i]:
                        ...
                        for d in range(1, length):
                            ...
                            if checked in self.tgt_positions or checked in self.box_positions or self.obstacle_map[checked[0]][checked[1]] \
                                    is self.OBSTACLE_SYMBOL or (self.obstacle_map[wall1[0]][wall1[1]] \
